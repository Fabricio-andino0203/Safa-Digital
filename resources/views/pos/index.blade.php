            ...
            <div class="p-5 space-y-2 max-h-96 overflow-y-auto">
                <template x-for="variante in productoSeleccionado?.variantes ?? []" :key="variante.id">
                    <button @click="agregarVariante(variante); modalVariante = false"
                            :disabled="productoSeleccionado?.controlar_stock && variante.stock_disponible <= 0"
                        ...
                        <div class="text-right flex-shrink-0 ml-4">
                            ...
                            <p class="text-xs" :class="!productoSeleccionado?.controlar_stock ? 'text-blue-500' : (variante.stock_disponible > 0 ? 'text-neutral-400' : 'text-red-500')"
                               x-text="!productoSeleccionado?.controlar_stock ? '∞ Bajo Pedido' : (variante.stock_disponible > 0 ? (variante.stock_disponible === 999999 ? '∞' : variante.stock_disponible) + ' disp.' : 'Sin stock')"></p>
                        </div>
                    </button>
                </template>
            </div>
                        ...
                        <div class="p-4 w-full flex flex-col flex-1">
                            ...
                            <div class="mt-auto flex items-end justify-between">
                                ...
                                <span class="text-[11px] font-semibold px-2 py-1 rounded-md"
                                      :class="!p.controlar_stock ? 'text-blue-600 bg-blue-50 border border-blue-200' : 'text-neutral-500 bg-neutral-100 border border-neutral-200'"
                                      x-text="!p.controlar_stock ? 'Bajo Pedido' : p.variantes.reduce((s,v) => s + (v.stock_disponible === 999999 ? 0 : v.stock_disponible), 0) + ' disp.'"></span>
                            </div>
                        </div>
                    </button>
                </template>
