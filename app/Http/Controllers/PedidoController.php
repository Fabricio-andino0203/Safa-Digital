...
class PedidoController extends Controller
{
    ...
    public function abonar(Request $request, $id)
    {
        ...
        try {
            ...
            $sesion = \App\Models\CajaSesion::sesionAbierta();
            if (!$sesion) {
                return response()->json(['success' => false, 'message' => 'Debe abrir una sesión de caja (en la pantalla de POS) para poder registrar pagos.'], 422);
            }
            ...
            if ($monto > $pedido->saldo_pendiente) {
                return response()->json(['success' => false, 'message' => 'El monto no puede superar el saldo pendiente.'], 422);
            }

            $pedido->total_abonado += $monto;
            ...
            $mov = CajaMovimiento::create([
                ...
                'tipo'           => 'ingreso',
                'monto'          => $monto,
                'concepto'       => 'Abono Pedido #' . $pedido->numero_orden . ($request->referencia ? ' (Ref: ' . $request->referencia . ')' : ''),
            ...
            return response()->json([
                'success' => true, 
                'pedido' => $pedido->load(['cliente', 'detalles.variante.producto', 'archivos']),
                'ticket_url' => route('pago.ticket', $mov->id)
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => $e->getMessage()], 422);
    ...
    public function liquidar(Request $request, $id)
    {
        ...
        try {
            ...
            if ($monto <= 0) {
                return response()->json(['success' => false, 'message' => 'El pedido ya está liquidado.'], 422);
            }

            $pedido->total_abonado += $monto;
            $pedido->saldo_pendiente = 0.00;
            $pedido->save();
            ...
            $mov = CajaMovimiento::create([
                ...
                'tipo'           => 'ingreso',
                'monto'          => $monto,
                'concepto'       => 'Liquidación Pedido #' . $pedido->numero_orden . ($request->referencia ? ' (Ref: ' . $request->referencia . ')' : ''),
                'referencia'     => strtoupper($request->metodo_pago),
                'pedido_id'      => $pedido->id,
                'fecha'          => now()->toDateString(),
            ]);
    ...
    public function entregarYLiquidar(Request $request, $id)
    {
        $request->validate([
            'metodo_pago' => 'required|in:efectivo,transferencia,tarjeta',
            'referencia' => 'nullable|string|max:255',
        ...
        try {
            ...
            if ($monto > 0) {
                ...
                $mov = CajaMovimiento::create([
                    ...
                    'tipo'           => 'ingreso',
                    'monto'          => $monto,
                    'concepto'       => 'Entrega y Liquidación Pedido #' . $pedido->numero_orden . ($request->referencia ? ' (Ref: ' . $request->referencia . ')' : ''),
                    'referencia'     => strtoupper($request->metodo_pago),
                    'pedido_id'      => $pedido->id,
                    'fecha'          => now()->toDateString(),
                ]);
                $primerMovimientoId = $mov->id;
            }

            // Cambiar estado a Entregado y procesar inventario
            ...
            return response()->json([
                'success' => true, 
                'pedido' => $pedido->load(['cliente', 'detalles.variante.producto', 'archivos']),
                'ticket_url' => $primerMovimientoId ? route('pago.ticket', $primerMovimientoId) : null
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => $e->getMessage()], 422);
